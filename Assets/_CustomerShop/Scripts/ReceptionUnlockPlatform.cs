...
public class ReceptionUnlockPlatform : MonoBehaviour
{
    ...
    {
        _receptionDesk.PaymentSuccessfulAction -= OnPaymentSuccessful;
    }

    public void Init(ReceptionDesk receptionDesk)
    {
        _receptionDesk = receptionDesk;
        
        SetCashText();

        if (_receptionDesk.IsUnlocked)
        {
    ...
    private bool DoPayment()
    {
        ...
        if (!_receptionDesk.CanDeposit(paymentsAmount, _currencyInTransit))
        {
            toDeposit = _receptionDesk.LastDepositAmount(_currencyInTransit);
        ...
        cashStack.DOMove(depositTarget.position, depositTravelTime).OnComplete(() =>
        {
            ...
            _receptionDesk.Deposit(toDeposit);
            SetCashText();
